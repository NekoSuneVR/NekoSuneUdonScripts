...
namespace NekoSune.WorldAvatarSearch.Editor
{
    ...
    internal sealed class NekoAvatarSearchBuilderWindow : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            EditorGUILayout.LabelField("A normal InputField cannot be converted into an arbitrary VRCUrl at runtime. The browser therefore supports creator-predeclared preset/demo URLs and a VRCUrlInputField where a user can enter a complete search URL. If the API is not trusted by VRChat, Allow Untrusted URLs is required.", NekoStyles.WrapLabel);
            EditorGUILayout.EndVertical();
        ...
        void BuildDemo()
        {
            ...
            if (urlInputType == null || pedestalType == null) { EditorUtility.DisplayDialog("Avatar Search", "VRChat Worlds SDK components were not found.", "OK"); return; }
            ...
            root.GetComponent<RectTransform>().sizeDelta = new Vector2(1320f, 900f); root.transform.localScale = Vector3.one * 0.001f;
            root.GetComponent<Image>().color = new Color(.025f,.032f,.055f,.985f); TryAddVrcUiShape(root);

            Text header = TextNode(root.transform,"Header","AVATAR SEARCH",44,FontStyle.Bold,TextAnchor.MiddleLeft); SetRect(header.rectTransform,new Vector2(.03f,.90f),new Vector2(.48f,.98f));
            Text sub = TextNode(root.transform,"SubHeader","VRCX-style JSON browser • flexible endpoint mapper",20,FontStyle.Normal,TextAnchor.MiddleLeft); SetRect(sub.rectTransform,new Vector2(.03f,.86f),new Vector2(.55f,.91f));

            GameObject urlBox = UiDynamic("[UrlInput]",root.transform,new[]{typeof(Image),typeof(InputField),urlInputType}); SetRect(urlBox.GetComponent<RectTransform>(),new Vector2(.03f,.79f),new Vector2(.66f,.85f)); urlBox.GetComponent<Image>().color=new Color(.08f,.10f,.15f,1f);
            InputField input=urlBox.GetComponent<InputField>(); Text inputText=TextNode(urlBox.transform,"Text","",19,FontStyle.Normal,TextAnchor.MiddleLeft); Stretch(inputText.rectTransform,12f); input.textComponent=inputText;
            Text placeholder=TextNode(urlBox.transform,"Placeholder","Paste complete search API URL...",18,FontStyle.Italic,TextAnchor.MiddleLeft); placeholder.color=new Color(.55f,.58f,.68f,1f); Stretch(placeholder.rectTransform,12f); input.placeholder=placeholder; input.navigation=new Navigation{mode=Navigation.Mode.None};
            ButtonNode(root.transform,"[SearchButton]","SEARCH",new Vector2(.675f,.79f),new Vector2(.79f,.85f)); ButtonNode(root.transform,"[DemoButton]","DEMO RINDO",new Vector2(.80f,.79f),new Vector2(.97f,.85f));

            GameObject listPanel=Ui("Results",root.transform,typeof(Image)); SetRect(listPanel.GetComponent<RectTransform>(),new Vector2(.03f,.10f),new Vector2(.56f,.76f)); listPanel.GetComponent<Image>().color=new Color(.045f,.055f,.085f,1f);
            var layout=listPanel.AddComponent<VerticalLayoutGroup>(); layout.padding=new RectOffset(14,14,14,14); layout.spacing=8f; layout.childControlWidth=true; layout.childForceExpandHeight=false;
            for(int i=0;i<8;i++) CreateResultCard(listPanel.transform,i);

            GameObject detail=Ui("Details",root.transform,typeof(Image)); SetRect(detail.GetComponent<RectTransform>(),new Vector2(.58f,.10f),new Vector2(.97f,.76f)); detail.GetComponent<Image>().color=new Color(.055f,.067f,.105f,1f);
            Text selectedName=TextNode(detail.transform,"[SelectedName]","NO AVATAR SELECTED",34,FontStyle.Bold,TextAnchor.UpperLeft); SetRect(selectedName.rectTransform,new Vector2(.06f,.78f),new Vector2(.94f,.94f));
            Text selectedAuthor=TextNode(detail.transform,"[SelectedAuthor]","",21,FontStyle.Bold,TextAnchor.UpperLeft); selectedAuthor.color=new Color(.4f,.68f,1f); SetRect(selectedAuthor.rectTransform,new Vector2(.06f,.70f),new Vector2(.94f,.80f));
            Text selectedDesc=TextNode(detail.transform,"[SelectedDescription]","Search for an avatar to begin.",19,FontStyle.Normal,TextAnchor.UpperLeft); selectedDesc.horizontalOverflow=HorizontalWrapMode.Wrap; selectedDesc.verticalOverflow=VerticalWrapMode.Truncate; SetRect(selectedDesc.rectTransform,new Vector2(.06f,.37f),new Vector2(.94f,.69f));
            Text selectedId=TextNode(detail.transform,"[SelectedId]","",14,FontStyle.Normal,TextAnchor.UpperLeft); selectedId.color=new Color(.62f,.65f,.72f); SetRect(selectedId.rectTransform,new Vector2(.06f,.29f),new Vector2(.94f,.37f));
            ButtonNode(detail.transform,"[PreviewButton]","PREVIEW",new Vector2(.06f,.15f),new Vector2(.46f,.25f)); ButtonNode(detail.transform,"[UseButton]","USE AVATAR",new Vector2(.50f,.15f),new Vector2(.94f,.25f));
            Text note=TextNode(detail.transform,"Note","Preview pedestal is shared world content. Avatar switching only affects you.",15,FontStyle.Normal,TextAnchor.UpperLeft); note.color=new Color(.65f,.68f,.75f); SetRect(note.rectTransform,new Vector2(.06f,.04f),new Vector2(.94f,.13f));
            Text status=TextNode(root.transform,"[StatusText]","Paste a search URL or press DEMO RINDO.",17,FontStyle.Normal,TextAnchor.MiddleLeft); status.color=new Color(.64f,.70f,.82f); SetRect(status.rectTransform,new Vector2(.03f,.02f),new Vector2(.97f,.08f));

            GameObject pedestalGo = new GameObject("Neko Avatar Search Preview Pedestal", pedestalType); Undo.RegisterCreatedObjectUndo(pedestalGo,"Create avatar preview pedestal"); pedestalGo.transform.position=root.transform.position+root.transform.right*1.7f;
            Component pedestal=pedestalGo.GetComponent(pedestalType); SetMember(pedestal,"ChangeAvatarsOnUse",true); SetMember(pedestal,"scale",.85f);
        ...
        {
            GameObject card=Ui("[Result"+index+"]",parent,typeof(Image),typeof(Button),typeof(LayoutElement)); card.GetComponent<Image>().color=index%2==0?new Color(.075f,.09f,.135f,1f):new Color(.065f,.078f,.118f,1f); card.GetComponent<LayoutElement>().preferredHeight=68f;
            Text name=TextNode(card.transform,"[Result"+index+"Name]","Avatar Result",21,FontStyle.Bold,TextAnchor.UpperLeft); SetRect(name.rectTransform,new Vector2(.03f,.45f),new Vector2(.72f,.94f));
            Text author=TextNode(card.transform,"[Result"+index+"Author]","by creator",15,FontStyle.Normal,TextAnchor.LowerLeft); author.color=new Color(.63f,.68f,.78f); SetRect(author.rectTransform,new Vector2(.03f,.08f),new Vector2(.72f,.48f));
            Text desc=TextNode(card.transform,"[Result"+index+"Description]","",12,FontStyle.Normal,TextAnchor.MiddleLeft); desc.gameObject.SetActive(false);
            Text status=TextNode(card.transform,"[Result"+index+"Status]","PUBLIC",13,FontStyle.Bold,TextAnchor.MiddleCenter); status.color=new Color(.45f,.86f,.58f); SetRect(status.rectTransform,new Vector2(.75f,.22f),new Vector2(.97f,.78f)); card.SetActive(false);
        }
        ...
        void AutoWire(GameObject root)
        {
            ...
            Type runtimeType=FindType("NekoAvatarSearchBrowser"); Type urlInputType=FindType("VRC.SDK3.Components.VRCUrlInputField"); Type pedestalType=FindType("VRC.SDK3.Components.VRCAvatarPedestal"); if(pedestalType==null)pedestalType=FindType("VRC.SDKBase.VRC_AvatarPedestal"); Type urlType=FindType("VRC.SDKBase.VRCUrl");
            ...
            Set(c,"searchUrlInput",Find(root,"[UrlInput]").GetComponent(urlInputType)); if(urlType!=null)Set(c,"demoSearchUrl",Activator.CreateInstance(urlType,new object[]{DemoUrl}));
            ...
            GameObject[] cards=new GameObject[8];Text[] names=new Text[8];Text[] authors=new Text[8];Text[] descs=new Text[8];Text[] statuses=new Text[8];
            for(int i=0;i<8;i++){cards[i]=Find(root,"[Result"+i+"]");names[i]=Find(root,"[Result"+i+"Name]").GetComponent<Text>();authors[i]=Find(root,"[Result"+i+"Author]").GetComponent<Text>();descs[i]=Find(root,"[Result"+i+"Description]").GetComponent<Text>();statuses[i]=Find(root,"[Result"+i+"Status]").GetComponent<Text>();Wire(cards[i].GetComponent<Button>(),c,"Select"+i);}
            Set(c,"resultCards",cards);Set(c,"resultNames",names);Set(c,"resultAuthors",authors);Set(c,"resultDescriptions",descs);Set(c,"resultStatus",statuses);Set(c,"selectedName",Find(root,"[SelectedName]").GetComponent<Text>());Set(c,"selectedAuthor",Find(root,"[SelectedAuthor]").GetComponent<Text>());Set(c,"selectedDescription",Find(root,"[SelectedDescription]").GetComponent<Text>());Set(c,"selectedId",Find(root,"[SelectedId]").GetComponent<Text>());Set(c,"statusText",Find(root,"[StatusText]").GetComponent<Text>());
            ...
            EditorUtility.SetDirty(c);EditorUtility.DisplayDialog("Avatar Search","Auto-wired. DEMO RINDO uses the predeclared endpoint; arbitrary searches use the complete-URL field because VRChat cannot construct arbitrary VRCUrls from a normal query string.","OK");
        }

        static GameObject Ui(string name,Transform parent,params Type[] components){return UiDynamic(name,parent,components);}
        static GameObject UiDynamic(string name,Transform parent,Type[] components){Type[] all=new Type[components.Length+1];all[0]=typeof(RectTransform);Array.Copy(components,0,all,1,components.Length);GameObject go=new GameObject(name,all);go.transform.SetParent(parent,false);return go;}
