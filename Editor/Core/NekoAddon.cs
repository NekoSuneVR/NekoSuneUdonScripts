...
namespace NekoSune.Worlds.Editor
{
    ...
    /// <summary>Public addon contract used by separately packaged World addons.</summary>
    ...
    internal static class NekoAddonRegistry
    {
        ...
        static void Scan()
        {
            ...
            for (int a = 0; a < assemblies.Length; a++)
            {
                ...
                {
                    Type type = types[i];
                    if (type == null || type.IsAbstract || type.IsInterface || !typeof(INekoAddon).IsAssignableFrom(type)) continue;
                    ...
                    catch (Exception e) { Debug.LogWarning("[NekoSune World Hub] Could not load addon " + type.FullName + ": " + e.Message); }
                }
            }
