...
namespace NekoSune.WorldStarterGames.Editor
{
    ...
    internal sealed class NekoStarterGamesWindow : EditorWindow
    {
        ...
        GameObject BuildFlappyUi()
        {
            ...
            GameObject bird = Panel(content.transform, "[Bird]", new Color(1f,.72f,.18f,1f)); RectTransform br = bird.GetComponent<RectTransform>(); br.anchorMin=br.anchorMax=new Vector2(.5f,.5f); br.sizeDelta=new Vector2(54,40); br.anchoredPosition=new Vector2(-220f,0f);
            for(int i=0;i<2;i++) CreatePipePair(content.transform,i, i==0?260f:650f);
            ButtonNode(content.transform,"[StartButton]","START",new Vector2(-110f,-280f));
            ButtonNode(content.transform,"[FlapButton]","FLAP",new Vector2(110f,-280f));
            return root;
        }
